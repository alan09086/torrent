...
mod tests {
    ...
    fn build_client_default_config() {
        let cfg = ssrf_config();
        let client = build_http_client(&cfg, None, "Ferrite/0.53.0");
    ...
    fn build_client_with_proxy() {
        let cfg = ssrf_config();
        let client = build_http_client(&cfg, Some("http://proxy.example.com:8080"), "Ferrite/0.53.0");
        drop(client);
    }
    ...
    fn build_client_invalid_proxy_fallback() {
        ...
        let client = build_http_client(&cfg, Some("not a url"), "Ferrite/0.53.0");
        drop(client);
    }
