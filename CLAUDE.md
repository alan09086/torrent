...
13-crate workspace: `torrent-bencode` → `torrent-core` → `torrent-wire`/`torrent-tracker`/`torrent-dht`/`torrent-storage`/`torrent-utp`/`torrent-nat` → `torrent-session` → `torrent` (facade) + `torrent-api` (HTTP) + `torrent-sim` (simulation)
...
- **torrent-api**: HTTP REST API (axum 0.8) — 19 endpoints + WebSocket event stream, delegates to `SessionHandle`, CLI `--api-port`
...
- `set_version(TorrentVersion)` — create v1, v2, or hybrid .torrent files
