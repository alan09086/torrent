...
- Current: v0.84.0, 1442 tests. Post-parity: M55 (speed), M56 (DHT persistence/stealing), M61 (perf optimizations), M65-M72 (CPU efficiency, adaptive queue, crypto optimization, memory/allocation, pipeline optimization, picker efficiency, steal phase optimization). M62 (semaphore pipeline) reverted due to data corruption. M73: per-peer autonomous dispatch (CPU -40%, speed regression). M74: dispatch stabilization — semaphore permit leak fixes. M75: peer-integrated dispatch — move requester into peer task's select! loop, eliminate cross-task permit lifecycle (56.7 MB/s, 4.6x over M74). M76: memory consolidation — remove PieceSelector/in_flight_pieces dead code, move peer_bitfields to peer-local, burst-mode connect interval (RSS -17%, cache misses -19%). M77: context switch reduction — remove notify wake storms from add_peer()/peer_have(), safety-net tick, batch drain 256→512 (54.2 MB/s, +3.2%). M78: actor throughput — notify_one() in complete_piece()/fail_piece(), two-phase dispatch (READ scan concurrent across peers), direct peer-to-disk writes via DiskHandle::enqueue_write() (RSS -9%, 81.2 MB). M81: ramp-up speed — three-phase connect interval (100ms/500ms/5s). M82: sustained throughput — adaptive max_in_flight, cached rarest-first dispatch, AIMD pipeline with permit absorption. M83: connection ramp-up elimination — default encryption Disabled (eliminates 23.4% CPU), AIMD initial depth 128 (eliminates 3s slow-start), SIGTERM handling for DHT persistence. 56.8 MB/s (+47%), CPU 13.1s (-29%), RSS 68.8 MiB (-27%). Gap to rqbit: 1.3x. M84: Have broadcast channel (371K→2.9K ops), pre-sized codec buffers, per-second stats logging. Peer-local block processing attempted but reverted (mutex contention slower than actor model).
